...
function createGUI() {
	...
	GUI.layMain.SetVisibility("Hide");
		...
		GUI.layStart = app.CreateLayout("absolute");
			...
			GUI.lstCategs.SetOnTouch(_ObjCbm(guiCallbacks.lstCategs_OnTouch,true));
		...
		GUI.layEdit.SetBackColor("#334433");
			...
			GUI.btnSaveDoc.SetOnTouch(_ObjCbm(guiCallbacks.btnSaveDoc_OnTouch));
			...
			GUI.edtAbbrev = app.CreateTextEdit("", .23, .08, "singleline");
			setModifier(GUI.edtAbbrev, guiCallbacks.edtAbbrev_OnModify);
			GUI.edtAbbrev.SetTextSize(15);
			GUI.edtAbbrev.SetBackColor("#445544");
			GUI.edtAbbrev.SetHint("Abbrev.");
			GUI.edtAbbrev.SetPosition(.715, .15);
			GUI.layEdit.AddChild(GUI.edtAbbrev);
			...
			GUI.edtShortDesc = app.CreateTextEdit("", .9, .095);
			setModifier(GUI.edtShortDesc, guiCallbacks.edtShortDesc_OnModify);
			GUI.edtShortDesc.SetBackColor("#445544");
			GUI.edtShortDesc.SetPosition(.05, .45);
			GUI.edtShortDesc.SetTextSize(15);
			GUI.edtShortDesc.SetHint("short description");
			GUI.layEdit.AddChild(GUI.edtShortDesc);

			GUI.txtP = app.CreateText("Parameters:");
			GUI.txtP.SetPosition(.05, .55);
			GUI.layEdit.AddChild(GUI.txtP);

			GUI.lstParams = app.CreateList("", .3, .3);
			GUI.lstParams.SetTextSize(15);
			GUI.lstParams.SetOnTouch(guiCallbacks.lstParams_OnTouch);
			...
			GUI.lstSubf = app.CreateList("", .55, .3, "html");
			GUI.lstSubf.SetTextSize(15);
			GUI.lstSubf.SetOnTouch(guiCallbacks.lstSubf_OnTouch);
			...
			GUI.edtPreview = app.CreateTextEdit("Preview", .9, -1, "singleline,readonly");
			GUI.edtPreview.SetBackColor("#445544");
			GUI.edtPreview.SetPosition(.05, .9);
		...
		GUI.laySamples.SetBackColor("#333344");
			...
			GUI.edtSCode.SetOnChange(function() { changed = true; });
	...
	app.AddLayout(GUI.layMain);
		...
		// adjust position and size dependent on label size
	GUI.txtP.h = 1*GUI.txtP.GetHeight();
	GUI.lstParams.SetPosition(.05, .55 + GUI.txtP.h, .3, .31 - GUI.txtP.h);
	
	GUI.txtSf.h = 1*GUI.txtSf.GetHeight();
	GUI.lstSubf.SetPosition(.4, .55 + GUI.txtSf.h, .55, .31 - GUI.txtSf.h);
	
	GUI.edtPreview.SetPosition(.05, .55 + GUI.txtSf.h + .33 - GUI.txtSf.h);
...
guiCallbacks = {
	...
	btnDocPreview_OnTouch: _ObjCbm(function() {
		Dlgs.Preview.Load("file:// " + path + "docs" + getl() + "/Docs.htm");
	}, true),
		...
		if(cat) {
			switch(title) {
				...
				case "+":
					...
					if(functions[title]) app.ShowPopup('imported "app.' + title + '"!');
					else {
						functions[title] = emptyFunc(title);
	...
	lstCategs_OnAction: function(action, force) {
			...
			case "Remove":
				...
				var onReply = function(reply) {
					if(reply == "Yes") {
						if(cat) {
						    if(cat == "All") {
						        delete functions[GUI.lstCategs.title];
		                        for(var i in categories) 
			                        delete categories[i][GUI.lstCategs.title];
						        saveFunctions();
						    } else
						        delete categories[cat][GUI.lstCategs.title];
                        } else
                            delete categories[GUI.lstCategs.title];
					}
                ...
                if(force) onReply("Yes");
                else {
                        Dlgs.Confirm.show(
    				    "Remove " + (cat? GUI.lstCategs.title + " from " + cat :
    				    GUI.lstCategs.title) + " section?\nATTENTION: " +
    				    "DELETING FROM ALL WILL REMOVE THE FUNCTION COMPLETELY!",
	...
	btnSaveDoc_OnTouch: function(onlysave) {
		GUI.edtDesc.fireCallback();
		GUI.edtShortDesc.fireCallback();
		GUI.edtAbbrev.fireCallback();
		
		if(!onlysave) {
			showProgress("generating doc file...");
			LoadDetailed();
			if(!app.FolderExists(path + "docs" + getl() + "/app")) app.MakeFolder(path + "docs" + getl() + "/app");
		    generateDoc(doc? doc : curF.name);
			loadDoc(curF);
		}
		
		showProgress("saving...");
		curF.abbrev = GUI.edtAbbrev.GetText();
		curF.shortDesc = GUI.edtShortDesc.GetText();

		saveFunctions();
		
		var info = JSON.parse(app.ReadFile(path + "info.json") || "{}");
		if(curF.isFunc && curF.abbrev != GUI.edtAbbrev.temp) {
			info[curF.abbrev] = info[GUI.edtAbbrev.temp];
			delete info[GUI.edtAbbrev.temp];
		}
		info.app[curF.name] = curF.shortDesc;
		app.WriteFile(path + "info.json", tos(info));
		delete info;
		
		if(!onlysave) app.ShowPopup("Saved and generated");
	...
	btnPreview_OnTouch: _ObjCbm(function() {
		var url = path + "docs/app" + getl() + "/" + (doc? doc : curF.name) + ".htm";
		if(!app.FileExists(url)) guiCallbacks.btnSaveDoc_OnTouch();
		Dlgs.Preview.Load("file:// " + url);
	}, true),
	...
	edtShortDesc_OnModify: function(txt) {
		if(txt == undefined) txt = GUI.edtShortDesc.GetText();
		else GUI.edtShortDesc.SetText(txt);
		if(doc) functions[doc].subf[curF.name].shortDesc = txt;
		else curF.shortDesc = txt;
		GUI.edtShortDesc.tmt = null;
		changed = true;
	},
	
	edtAbbrev_OnModify: function(txt) {
		if(txt == undefined) txt = GUI.edtAbbrev.GetText();
		else GUI.edtAbbrev.SetText(txt);
		if(doc) functions[doc].subf[curF.name].abbrev = txt;
		else curF.abbrev = txt;
		GUI.edtAbbrev.tmt = null;
		changed = true;
	},
...
*/
	lstSamples_OnAction: function(action, title, body, index) {
		switch(action) {
			...
			case "Move":
					// create new object with changed key order
				var lst = keys(GUI.lstSamples.samples), obj = {};
			...
			case "Insert":
				...
				GUI.edtSCode.SetText(GUI.lstSamples.samples[title] = "");
	...
	lstSamples_OnTouch: _ObjCbm(function(title) {
		...
		if(changed)
			Dlgs.Confirm.show("Save \"" + sName + "\" first?", function(reply, btn) {
				if(reply == "Yes") guiCallbacks.btnSaveSamples_OnTouch(sName, GUI.edtSCode.GetText());
				loadSample(title);
			});
	...
	btnSaveSamples_OnTouch: function (nTitle, code) {
	        // ignore input if function not called from button
	    if(nTitle !== false && code !== false) {
    			// get default value
    		if(!nTitle) nTitle = GUI.edtSName.GetText();
    		if(!code) code = GUI.edtSCode.GetText();

    			// check wether title and code are defined
    		if(!nTitle || !code)
    			return app.Alert("please set a title and write some code", "", 255);
    		...
    		if(nTitle != false) {
    			...
    			if(GUI.lstSamples.title != nTitle) {
    			    if(GUI.lstSamples.samples[nTitle]) return alert("Sample '" + nTitle + "' already exists!");
    					// delete old
    				delete GUI.lstSamples.samples[GUI.lstSamples.title];
    					// save new
    				GUI.lstSamples.title = nTitle;
    				GUI.lstSamples.samples[nTitle] = code.trim();
    					// save
    			} else GUI.lstSamples.samples[nTitle] = code.trim();
		...
		var samples = "";
		for(var title in GUI.lstSamples.samples)
