...
function createDlgs() {
	Dlgs.Language = app.CreateDialog( "Choose your preferred language", "NoCancel" );
	Dlgs.Language.SetOnBack( dlgCallbacks.languageOnBack );
	Dlgs.Language.show = function( callback, back ) {
		Dlgs.Language.callback = callback || null;
		Dlgs.Language.back = back == undefined? true : back;
	...
	Dlgs.Export.show = function( callback ) {
		...
		Dlgs.Export.lay.SetPadding( .1, .05, .1, .05 );
			...
			Dlgs.Export.chkGens = app.CreateCheckBox( "generation files (recommented)" );
	...
	Dlgs.Readme.SetOnCancel( function() { useprg = true; } );
		Dlgs.Readme.page = 1;
		Dlgs.Readme.readme = app.ReadFile( "README.txt" ).replace( /\t/g, "  " )
			.replace( /\n/g, "<br>" ).replace( /  /g, "&#160;&#160;" )
			.replace( /"white"/g, "\"#ffffff\"" ).replace( /<br> /g, "<br>&#160" );
		Dlgs.Readme.license = app.ReadFile( "LICENSE.txt" ).replace( /\t/g, "  " )
			.replace( /\n/g, "<br>" ).replace( /  /g, "&#160;&#160;" )
			.replace( /<br> /g, "<br>&#160" );
		Dlgs.Readme.lay = app.CreateLayout( "Linear" );
			...
			Dlgs.Readme.scr.SetMargins( .03, .02, .03, .02 );
				Dlgs.Readme.txt = app.CreateText( Dlgs.Readme.license, -1, -1, "left,html,multiline,monospace" );
				Dlgs.Readme.txt.SetTextSize( 10 );
				Dlgs.Readme.scr.AddChild( Dlgs.Readme.txt );
				Dlgs.Readme.lay.AddChild( Dlgs.Readme.scr );

				Dlgs.Readme.btn = app.CreateButton( "Readme", -1, .08 );
	...
	Dlgs.PType.show = function( callb ) {
		...
		Dlgs.PType.layS = app.CreateLayout( "Linear" );
			Dlgs.PType.layS.SetVisibility( "Hide" );
				...
				Dlgs.PType.edt = app.CreateTextEdit( "", .9, .5 );
			...
			Dlgs.PType.lst = app.CreateList( listTypes, .9, .8 );
...
var dlgCallbacks = {
	...
	btnReadme_OnTouch: function() {
		...
		Dlgs.Readme.btn.SetText( Dlgs.Readme.page? "License" : "Readme" );
		if( Dlgs.Readme.read == undefined )
			Dlgs.Readme.read = true;
	},
	...
	webPreview_OnProgress: _ObjCbm( function() {
		...
		Dlgs.Preview.web.tmt = setTimeout( function() {
			Dlgs.Preview.Show(); hideProgress();
		}, 500 );
	}, true ),
	...
	dlgTypesLst_OnTouch: _ObjCbm( function( desc, type ) {
		...
		if( Dlgs.PType.lst.main ) {
			...
			if( desc.indexOf( ">" ) > -1 ) {
				...
				Dlgs.Confirm.show( "Do you want to specify\nthe possible values?", function( reply ) {
					if( reply == "Yes" ) {
			 ...
			 if( ["num", "str", "lst", "obj", "fnc"].indexOf( type.slice( 0, 3 ) ) > -1 ) {
				Dlgs.Confirm.show( "Do you want to specify the possible values?", function( reply ) {
					if( reply == "Yes" ) {
						Dlgs.PType.sTitle.SetHtml( "<font size=\"15pt\"><b>" + type + "</b><font><br><font size=\"10pt\">" + ( hrefs[type] || typenames[type.slice( 0,3 )] ) + "</font>");
						Dlgs.PType.lay.Animate( "Flip", null, 300 );
					} else {
